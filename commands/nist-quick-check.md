...
description: Orchestrated NIST quick check of a specific control or family with durable artifacts and a cold review pass
...
Run a focused NIST quick check using one domain assessor plus one condensed cold review.

Reference the shared contract in `references/orchestration-contract.md`.

## Scope Rules

Valid NIST quick-check families:
...
Map the request to the responsible assessor:
...
2. Identify the responsible assessor.
...
- any conditional applicability note for CM, RA, or SA
...
Dispatch the responsible NIST domain assessor with the scoped controls only.
...
Run one fresh reviewer cold. Use `control-interpretation-reviewer` unless another reviewer is clearly better for the requested control family.
